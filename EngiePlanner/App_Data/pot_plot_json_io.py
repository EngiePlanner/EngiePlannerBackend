...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%\n'''
        ...
        for task in self.json_data['tasks']:
            if task['duration'] <= 38:
                lp += 'task(({}, {})). '.format(task['name'], task['delivery'])
                lp += 'task_available_day(({}, {}),{}). '.format(task['name'], task['delivery'], self.integer_dates[task['start_date']])
                available_week = int(np.ceil(self.integer_dates[task['start_date']]/5))
                lp += 'task_available({},{}). '.format(task['delivery'], available_week)
                lp += 'task_planned_day(({}, {}),{}). '.format(task['name'], task['delivery'], self.integer_dates[task['planned_date']])
        ...
        for availability in self.availability_data:
            ...
            lp += 'capacity({}, {}, {}). '.format(availability['name'], week, availability['availableHours'])
            lp += '\n'
...
class JsonOutputWriter():
    def __init__(self, answer_set, date2integer_dict, output_file_name = 'clingo_output.json'):          
        ...
        self._write_json(self.answer_set_list_start_end, 'start_end.json')
        #embed()
    ...
    def _fill_dictionary(self):
        for atom in self.answer_set:
            ...
            if 'assign_day' in atom:
                ...
                assign_day = re.findall(regex, atom) 
                temp_day = self.schedule_start_day + datetime.timedelta(days=int(assign_day[0][1]))
                temp_date = temp_day.strftime('%d.%m.%Y')
                temp_dict = {'assignee' : assign_day[0][0],
                                'day' : temp_date,
                                'workitem' : assign_day[0][2].split(',')[0],
    ...
    def _start_end_dict(self):
        ...
        for atom in self.answer_set:
            if 'start_day' in atom:
                ...
                start_day = re.findall(regex, atom) 
                #print(atom)
                for atom2 in temp_answer_set:
                    if 'end_day' in atom2:
                        #print('end ' + atom2)
                        regex_end = r'end_day\(([A-Za-z0-9]+),([0-9]+),\(([A-Za-z0-9,_]+)\)\)'
                        end_day = re.findall(regex_end, atom2) 
                        #embed()
                        if start_day[0][2] == end_day[0][2]:
                            
                            temp_day_start = self.schedule_start_day + datetime.timedelta(days=int(start_day[0][1]))
                            temp_day_end = self.schedule_start_day + datetime.timedelta(days=int(end_day[0][1]))
                            temp_date_start = temp_day_start.strftime('%d.%m.%Y')
                            temp_date_end = temp_day_end.strftime('%d.%m.%Y')
                            #embed()
                            temp_dict = {'Resource' : start_day[0][0],
